                                ...
                                <div class="list_category list">
                                    ...
                                    <nav class="navigation paging-navigation" role="navigation">
                                        ...
                                        {%if totalpage >0 %}
											...
											{%if page > 1%}
												<a class="previouspostslink" rel="prev" href="{{url.get('c/'~page_no~'/'~(page-1))}}">«</a>
												{%endif%}
											{%for rowpage in 1..totalpage %}
												{%if rowpage == page%}
													<span class='current'>{{page}}</span>
												{%else%}
													<a class="page larger" href="{{url.get('c/'~page_no~'/'~rowpage)}}">{{rowpage}}</a>
												{%endif%}
											{%endfor%}
											{%if page < totalpage%}
											<a class="nextpostslink" rel="next" href="{{url.get('c/'~page_no~'/'~(page+1))}}">»</a>
											{%endif%}
										{%endif%}
										</div>                                    
									</nav><!-- .navigation -->
